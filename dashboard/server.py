"""
dashboard/server.py — SHADOW Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "SHADOW Uploads",
        Path.home() / "Documents" / "SHADOW Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'SHADOW-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"SHADOW Dashboard Port {port}"
        prog_rule  = "SHADOW Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="shadow_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until SHADOW is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "shadow.key").exists() and (certs / "shadow.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in SHADOW to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('shadow_token','{tok}');
  sessionStorage.setItem('shadow_key','{key}');
  localStorage.setItem('shadow_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to SHADOW…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "shadow.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "shadow.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in SHADOW UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
