...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "shadow"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("shadow:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for SHADOW", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="SHADOW", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "SHADOW"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Shadow actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — SHADOW ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "SHADOW").strip()
        ...
        self.setWindowTitle(f"{_display} — AI Voice Assistant")
        ...
        self.on_interrupt      = None   # callable: () -> None — stop SHADOW mid-speech
        self._muted            = False
        self._current_file: str | None = None
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_shadow_icon(out_path: Path) -> bool:
        """
        Render a SHADOW arc-reactor icon at 4× resolution and downsample
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        """
        Create a Windows .lnk shortcut without opening a console.

        For packaged ShadowAI, the shortcut uses the icon embedded
        inside ShadowAI.exe. This prevents Windows from falling back
        to another/default icon.
        """

        # For a packaged PyInstaller build, use the executable itself
        # as the icon source.
        if getattr(sys, "frozen", False):
            icon_loc = f"{Path(sys.executable).resolve()},0"
        ...
        try:
            ...
            sc.Description = "S.H.A.D.O.W AI Assistant"

            # IMPORTANT:
            # Use ShadowAI.exe's embedded icon.
    ...
    def _create_desktop_shortcut(self):
        ...
        """
        print("DEBUG: CREATE DESKTOP SHORTCUT CLICKED")
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "shadow.ico"
        if not ico_path.exists():
            self._build_shadow_icon(ico_path)
        ...
        try:
            ...
            if _os == "Windows":
                ...
                # Packaged ShadowAI → use the actual EXE
                if getattr(sys, "frozen", False):
                    ...
                    lnk = str(desktop / "S.H.A.D.O.W.lnk")

                    # Use the actual Shadow icon file
                    icon_loc = str(ico_path.resolve())

                    self._create_lnk_windows(
                        lnk,
                        target,
                        "" if getattr(sys, "frozen", False) else str(script),
                        work_dir,
                        icon_loc
                    )
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>SHADOW</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.shadow.assistant</string>\n'
                    '  <key>CFBundleName</key><string>S.H.A.D.O.W</string>\n'
            ...
            # ── Linux — .desktop file (Terminal=false, no console) ────────────
                ...
                desk = desktop / "S.H.A.D.O.W.desktop"
                desk.write_text(
                    "[Desktop Entry]\n"
                    "Name=S.H.A.D.O.W\n"
                    f"Exec={python} {script}\n"
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("SHADOW", C.PRI_DIM))
        lay.addSpacing(8)
        ...
        _sub_text = ("DEVELOPED BY AYAN NAGARI"
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By Ayan Nagari", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "SHADOW_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.shadow.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "shadow.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "SHADOW_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "SHADOW_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.shadow.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.shadow.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "SHADOW") or "SHADOW",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "SHADOW"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — SHADOW Personal AI Assistant")
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "SHADOW") or "SHADOW"
