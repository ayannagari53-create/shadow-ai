...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".shadow_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".shadow_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using SHADOW profile")
                shadow = str(Path.home() / ".shadow_profiles" / "firefox_shadow")
                Path(shadow).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(shadow, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".shadow_profiles" / "safari")
        ...
        # SHADOW otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        shadow_profile = str(Path.home() / ".shadow_profiles" / self.browser_name)
        Path(shadow_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with SHADOW profile: {shadow_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(shadow_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with SHADOW profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "shadow_screenshot.png")
